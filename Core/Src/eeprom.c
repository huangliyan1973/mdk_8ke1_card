...
#define ADDR_FLASH_SECTOR_11    (0x080E0000) 	//扇区11起始地址,128 Kbytes, 选用此扇区存储参数
...
void update_eeprom(void)
{
	...
	FlashEraseInit.Sector = FLASH_SECTOR_11;   				//要擦除的扇区
	FlashEraseInit.NbSectors = 1;                             //一次只擦除一个扇区
	FlashEraseInit.VoltageRange = FLASH_VOLTAGE_RANGE_3;      //电压范围，VCC=2.7~3.6V之间!!
	if(HAL_FLASHEx_Erase(&FlashEraseInit,&SectorError) != HAL_OK){
		CARD_PLATFORM_DIAG(("ERASE FLASH FAILED!\n"));
		return;
	}
	...
	if (flashStatus == HAL_OK){
		...
		for (uint16_t i = 0; i < cn_32; i++)
			HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, ADDR_FLASH_SECTOR_11+i*4, pbuf[i]);
	}
...
{
	e1_params = *(e1_params_t *)ADDR_FLASH_SECTOR_11;
}
...
void init_eeprom(void)
{
    ...
    if (e1_params.version != E1_CARD_VERSION) {
        CARD_DEBUGF(FLASH_DEBUG, ("Start default param set!\n"));
